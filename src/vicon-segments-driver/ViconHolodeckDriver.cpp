...
void printLCM(vicon::vicon_t& vicon)
{
    ...
    for(i = 0; i < vicon.nummodels; i++)
    {
        ...
        {
            vicon::segment_t& segment = model.segments[j];
...
int main(int argc, char** argv)
{
    ...
    if (argc < 2)
    {
        std::cout << "Usage: " << argv[0] << " <vicon-client-segments host:port ex. 192.168.10.1:801>" << std::endl;
        return 1;
    }
    ...
    while (1)
    {
        ...
        {
            vicon::model_t& model = findModel(system.models, vicon.GetSubjectName(i).SubjectName);
            ...
            {
                vicon::marker_t& marker = findMarker(model.markers, vicon.GetMarkerName(model.name, j).MarkerName);
            ...
            {
                vicon::segment_t& segment = findSegment(model.segments, vicon.GetSegmentName(model.name, j).SegmentName);
